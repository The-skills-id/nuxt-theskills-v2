<template>
  <v-app>
    ...
    <v-container v-show="!loading" grid-list-md>
      ...
      <v-row class="ml-3 mt-3"> {{ subcourses.description }} </v-row>
      <div class="ml-3 mt-6 title">
        Daftar Materi {{ subcourses.course_name }}
      </div>
        ...
        <v-expansion-panels accordion>
          ...
          >
            ...
            <v-expansion-panel-content
              v-for="(item, index) in item.minicourse"
              :key="index"
              class="caption"
            >
              <v-icon left> letter-b-box </v-icon>
              {{ item.minicourse_name }}
            </v-expansion-panel-content>
          </v-expansion-panel>
        </v-expansion-panels>
      </v-row>
